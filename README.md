...
- Updated `device_name` field to include the latest devices.
- Fixed rare local network permission prompt.
- Minor improvements.
...
- Removed error message when calling `startBehaviourMonitoring`. Now the SDK only displays a warning message when logging has been enabled. **On a Simulator the SDK can only use default sensor values, please use a phyiscal device to test and debug the behaviour monitoring feature.**
